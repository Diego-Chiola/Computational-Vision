...
Because optical flow estimates are noisy and sometimes fail (due to occlusion or blurring), we use a \textbf{Kalman Filter} to maintain robust tracks.
...
If Track 1 and Track 2 are close to each other, their prediction ellipses might intersect. When a new corner is detected in that intersection area, the system faces the \textbf{Data Association Problem}: determining which track the newly detected measurement actually belongs to.
